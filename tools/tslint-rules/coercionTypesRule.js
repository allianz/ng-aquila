...
class Walker extends Lint.RuleWalker {
    constructor(sourceFile, options, _typeChecker) {
        super(sourceFile, options);
        this._typeChecker = _typeChecker;
        /** Type resolving to the TS internal `null` type. */
        this._nullType = this._typeChecker.getNullType();
        /** Type resolving to the TS internal `undefined` type. */
        this._undefinedType = this._typeChecker.getUndefinedType();
    ...
    _lintCoercionMember(node) {
        ...
        if (!hasNull && !hasUndefined) {
            this.addFailureAtNode(node, 'Acceptance member has to accept "null" and "undefined".', this.appendText(node.type.getEnd(), ' | null | undefined'));
        }
        else if (!hasNull) {
     ...
     */
    _lintClass(node, sourceClass, expectDeclaredMembers) {
    ...
    _lintSuperClasses(node) {
        ...
        while (currentClass) {
            ...
            currentClass = symbol && symbol.valueDeclaration && ts.isClassDeclaration(symbol.valueDeclaration) ?
     ...
     * @param expectDeclaredMembers Whether acceptance members should be expected or unexpected.
     */
    _lintInterfaces(node, sourceClass, expectDeclaredMembers) {
            ...
            if (clause.token === ts.SyntaxKind.ImplementsKeyword) {
                clause.types.forEach(clauseType => {
                    if (ts.isIdentifier(clauseType.expression)) {
                        ...
                        if (propNames) {
                            propNames.forEach(propName => this._checkStaticMember(sourceClass, propName, expectDeclaredMembers));
                        }
                    }
                });
            }
     ...
     */
    _checkStaticMember(node, setterName, expectDeclaredMembers) {
        ...
        if (expectDeclaredMembers && !memberNode) {
            ...
            this.addFailureAtNode(node.name || node, `Class should not declare static coercion ` +
                `property called ${memberName}. Acceptance members are inherited.`, Lint.Replacement.deleteText(memberNode.getFullStart(), memberNode.getFullWidth()));
        }
    }
    /** Checks whether this rule should lint a class declaration. */
    _shouldLintClass(node) {
    ...
    _lookupStaticMember(node, propName) {
 ...
 */
function usesCoercion(setter, coercionFunctions) {
    let coercionWasUsed = false;
    setter.forEachChild(function walk(node) {
...
function getBaseTypeIdentifier(node) {
    if (node.heritageClauses) {
...
function isDecoratorCalled(node, name) {
