...
import {
  ...
  ElementRef, HostBinding, Input, Optional, QueryList, Self, ViewChild, ViewChildren
} from '@angular/core';
import {
...
export class NxMultiSelectComponent<S, T> implements ControlValueAccessor, NxFormfieldControl<T[]>, AfterViewInit {
  ...
  _onFilterChange(query: string) {
    if (!query) {
      ...
      this.listItems = this.options.filter(
        (item) => this._selectLabel(item).toLowerCase().indexOf(query.toLowerCase()) >= 0
      );
    }
  ...
  _onSelectAll() {
    if (this._allSelected) {
      ...
      this.options.filter(option => !this._isDisabled(option))
  ...
  private _initKeyManager() {
    this._keyManager = new ActiveDescendantKeyManager<
      NxMultiSelectOptionComponent<T>
    >(this._options)
      .withHomeAndEnd()
