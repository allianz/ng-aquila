...
const _defaultFilterFn: NxMultiSelectFilterFn = (query, label) =>
...
export class NxMultiSelectComponent<S, T>
  implements ControlValueAccessor, NxFormfieldControl<T[]>, DoCheck, OnDestroy, AfterViewInit
{
  get value(): T[] {
    return this.options
  ...
  private get _isActiveItemFiltered(): boolean {
    return (
      ...
      !this.listItems.some(
        (option) => this._selectValue(option) === this._keyManager.activeItem?.value,
      )
    );
  }
  ...
  _onFilterChange(query: string | null | undefined) {
    ...
    if (query) {
      this.listItems = [...this.options]
        .sort(this.sortSelectedToTop)
        .filter((item) => this.filterFn(query, this._selectLabel(item)));
    } else {
  ...
  private _updateTooltipText() {
    ...
    const triggerContentWidth =
      this._trigger.nativeElement.clientWidth -
