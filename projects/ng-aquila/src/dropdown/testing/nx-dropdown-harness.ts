...
export class NxDropdownHarness extends NxFormfieldControlHarness {
  ...
  static with(options: NxDropdownFilters = {}) {
    return new HarnessPredicate(NxDropdownHarness, options)
      .addOption('valueText', options.valueText, (harness, text) =>
  ...
  async getItems(
    filter?: Omit<NxDropdownItemFilters, 'ancestor'>,
  ): Promise<NxDropdownItemHarness[]> {
  ...
  async getGroups(
    filter?: Omit<NxDropdownGroupHarnessFilters, 'ancestor'>,
  ): Promise<NxDropdownGroupHarness[]> {
    return this.documentRootLocator.locatorForAll(
      // There is currently no way to specifically get this dropdown's panel body, so multiple instances of the dropdown harness could
  ...
  async clickItem(filter?: NxDropdownItemFilters): Promise<void> {
    ...
    if (items.length === 0) {
      ...
      const unfilteredItemTexts = (
        await parallel(() => unfilteredItems.map((o) => o.getText()))
      ).join(', ');
      throw Error(
        `Select does not have options matching the specified filter, available items: ${unfilteredItemTexts}.`,
      );
    }
