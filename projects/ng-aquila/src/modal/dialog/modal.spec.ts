...
describe('NxDialog', () => {
  ...
  beforeEach(inject([NxDialogService, Location, OverlayContainer],
    (d: NxDialogService, l: Location, oc: OverlayContainer) => {
      dialog = d;
  ...
  describe('closing', () => {
    ...
    it('should close from a ViewContainerRef with OnPush change detection', fakeAsync(() => {
      ...
      expect(overlayContainerElement.querySelectorAll('nx-modal-container').length)
  ...
  it('should override the max-width of the overlay pane', fakeAsync(() => {
    ...
    expect(overlayPane.style.maxWidth).toBe('736px',
  ...
  it('should close all of the dialogs', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('nx-modal-container').length).toBe(3);

    dialog.closeAll();
    viewContainerFixture.detectChanges();
    flush();

    expect(overlayContainerElement.querySelectorAll('nx-modal-container').length).toBe(0);
  }));

  it('should set the proper animation states', () => {
    const dialogRef = dialog.open(PizzaMsg, { viewContainerRef: testViewContainerRef });
    const dialogContainer: NxModalContainer =
        viewContainerFixture.debugElement.query(By.directive(NxModalContainer))!
        .componentInstance;
  ...
  it('should close all dialogs when the user goes forwards/backwards in history', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('nx-modal-container').length).toBe(2);
  ...
  it('should allow the consumer to disable closing a dialog on navigation', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('nx-modal-container').length).toBe(1);
  }));
  ...
  describe('focus management', () => {
    ...
    it('should move focus to the container if there are no focusable elements in the dialog',
      fakeAsync(() => {
        ...
        expect(document.activeElement!.tagName)
            .toBe('NX-MODAL-CONTAINER', 'Expected dialog container to be focused.');
      }));
  ...
  describe('dialog content elements', () => {
    ...
    function runContentElementTests() {
      it('should add the respective classes for content and action sections', fakeAsync(() => {
        expect(overlayContainerElement.querySelectorAll('.nx-modal__content').length).toBe(1);
        expect(overlayContainerElement.querySelectorAll('.nx-modal__actions').length).toBe(1);
      }));

      it('should close the dialog when clicking on the close button', fakeAsync(() => {
        expect(overlayContainerElement.querySelectorAll('.nx-modal__container').length).toBe(1);

        (overlayContainerElement.querySelector('button[nxModalClose]') as HTMLElement).click();
        viewContainerFixture.detectChanges();
        flush();

        expect(overlayContainerElement.querySelectorAll('.nx-modal__container').length).toBe(0);
      }));
      ...
      it('should return the [nxModalClose] result when clicking the close button',
        fakeAsync(() => {
          const afterCloseCallback = jasmine.createSpy('afterClose callback');
          dialogRef.afterClosed().subscribe(afterCloseCallback);
...
describe('NxDialog with a parent NxDialog', () => {
  ...
  it('should close dialogs opened by a parent when calling closeAll on a child NxDialog',
    fakeAsync(() => {
      ...
      expect(overlayContainerElement.textContent!.trim())
          .toBe('', 'Expected closeAll on child NxDialog to close dialog opened by parent');
    }));

  it('should close dialogs opened by a child when calling closeAll on a parent NxDialog',
    fakeAsync(() => {
      ...
      expect(overlayContainerElement.textContent!.trim())
          .toBe('', 'Expected closeAll on parent NxDialog to close dialog opened by child');
    }));
...
describe('NxDialog with default options', () => {
  ...
  beforeEach(fakeAsync(() => {
    ...
    TestBed.configureTestingModule({
      imports: [NxModalModule, DialogTestModule],
      providers: [
        {provide: NX_MODAL_DEFAULT_OPTIONS, useValue: defaultConfig},
      ],
    });
  ...
  beforeEach(inject([NxDialogService, OverlayContainer],
    (d: NxDialogService, oc: OverlayContainer) => {
      dialog = d;
      overlayContainer = oc;
      overlayContainerElement = oc.getContainerElement();
    }));
...
class PizzaMsg {
  constructor(public dialogRef: NxModalRef<PizzaMsg>,
