...
export const NX_MODAL_DEFAULT_OPTIONS =
  new InjectionToken<NxModalConfig>('nx-modal-default-options');

/** Injection token that determines the scroll handling while the modal is open. */
export const NX_MODAL_SCROLL_STRATEGY =
  new InjectionToken<() => ScrollStrategy>('nx-modal-scroll-strategy');
...
/** @docs-private */
export function NX_MODAL_SCROLL_STRATEGY_PROVIDER_FACTORY(overlay: Overlay):
  () => ScrollStrategy {
  return () => overlay.scrollStrategies.block();
}
...
export class NxDialogService implements OnDestroy {
  ...
  open<T, D = any, R = any>(componentOrTemplateRef: ComponentType<T> | TemplateRef<T>,
    config?: NxModalConfig<D>): NxModalRef<T, R> {
    ...
    const modalRef = this._attachModalContent<T, R>(componentOrTemplateRef,
      modalContainer,
      overlayRef,
      config);
  ...
  private _attachModalContainer(overlay: OverlayRef, config: NxModalConfig): NxModalContainer {
    ...
    const containerPortal = new ComponentPortal(NxModalContainer,
  ...
  private _attachModalContent<T, R>(
    ...
    config: NxModalConfig): NxModalRef<T, R> {

    // Create a reference to the modal we're creating in order to give the user a handle
    // to modify and close it.
    const modalRef =
      new NxModalRef<T, R>(overlayRef, modalContainer, config.id);
    ...
    if (componentOrTemplateRef instanceof TemplateRef) {
      modalContainer.attachTemplatePortal(
        new TemplatePortal<T>(componentOrTemplateRef, null!,
          { $implicit: config.data, modalRef } as any));
    } else {
  ...
  private _createInjector<T>(
    ...
    modalRef: NxModalRef<T>,
    modalContainer: NxModalContainer): Injector {
...
function _applyConfigDefaults(
  config?: NxModalConfig, defaultOptions?: NxModalConfig): NxModalConfig {
