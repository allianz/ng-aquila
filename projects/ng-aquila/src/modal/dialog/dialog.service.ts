...
export const NX_MODAL_DEFAULT_OPTIONS = new InjectionToken<NxModalConfig>(
  'nx-modal-default-options',
);

/** Injection token that determines the scroll handling while a modal is open. */
export const NX_MODAL_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'nx-modal-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
...
export const INERT_EXCEPTION_SELECTORS = new InjectionToken<string[]>(
  'Selector for element that should be exept from inert',
  {
    providedIn: 'root',
...
export class NxDialogService implements OnDestroy {
  ...
  get openModals(): NxModalRef<any>[] {
    return this._parentDialogService
      ? this._parentDialogService.openModals
  ...
  get ariaHiddenElements(): Map<Element, string | null> {
    return this._parentDialogService
      ? this._parentDialogService.ariaHiddenElements
  ...
  get afterOpened(): Subject<NxModalRef<any>> {
    return this._parentDialogService
      ? this._parentDialogService.afterOpened
  ...
  constructor(
    ...
    @Optional()
    @Inject(NX_MODAL_DEFAULT_OPTIONS)
    private readonly _defaultOptions: NxModalConfig | null,
