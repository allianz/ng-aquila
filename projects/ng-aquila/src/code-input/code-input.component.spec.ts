...
describe('NxCodeInputComponent', () => {
  ...
  it('should select second input on right arrow', fakeAsync(() => {
    ...
    expect(_getFocusedElementPierceShadowDom()).toEqual(
      codeInputElement.querySelector('input:nth-child(2)') as HTMLElement,
    );
  }));
  ...
  it('should overwrite the default aria labels', () => {
    ...
    Array.from(inputElements).forEach((inputEl, index) => {
      expect((inputEl as HTMLElement).getAttribute('aria-label')).toBe(
        `${index + 1} testOf ${inputElements.length}`,
      );
    });
  });
  ...
  describe('a11y', () => {
    ...
    it('should link error id with aria-descrideby', async () => {
      ...
      const ariaDescrideby = fixture.nativeElement
        .querySelector('.nx-code-input')
