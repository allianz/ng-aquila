...
import { NxWordComponent } from '@allianz/ng-aquila/natural-language-form';
...
export const NX_AUTOCOMPLETE_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'nx-autocomplete-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
 ...
 */
export function NX_AUTOCOMPLETE_SCROLL_STRATEGY_PROVIDER_FACTORY(
  overlay: Overlay,
): () => ScrollStrategy {
  return () => overlay.scrollStrategies.reposition();
}
...
export class NxAutocompleteTriggerDirective
  implements ControlValueAccessor, OnDestroy, OnChanges, AfterViewInit, OnInit
{
  ...
  private readonly _windowBlurHandler = () => {
    ...
    this._canOpenOnNextFocus =
      _getFocusedElementPierceShadowDom() !== this._element.nativeElement || this.panelOpen;
  };
  ...
  private _bindAutocompleteItems() {
    ...
    if (typeof this._itemsCb === 'function' && this.autocomplete && !this.autocompleteDisabled) {
      ...
      this._controlValueChangesSubscription = valueChanges
  ...
  _handleInput(event: KeyboardEvent): void {
    ...
    if (
      this._isFieldEnabled() &&
      this._previousValue !== value &&
      _getFocusedElementPierceShadowDom() === event.target
    ) {
      this._previousValue = value;
      this._onChange(value);
      this.openPanel();
