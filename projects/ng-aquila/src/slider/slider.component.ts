...
export class NxSliderComponent implements ControlValueAccessor, AfterViewInit, OnDestroy {
  ...
  _changeValue(valueDiff: number) {
    ...
    if (this._isValidStep()) {
      ...
      newValue =
        valueDiff < 0
          ? minAdjustedValue.toNearest(this.step, Decimal.ROUND_DOWN)
  ...
  _dragStart() {
    ...
    this._ngZone.runOutsideAngular(() => {
      this._dragSubscriptions.push(
        fromEvent<TouchEvent>(document, 'touchmove').subscribe(this._handleDragMove.bind(this)),
      );
      this._dragSubscriptions.push(
        fromEvent<MouseEvent>(document, 'mousemove').subscribe(this._handleDragMove.bind(this)),
      );
    });

    this._dragSubscriptions.push(
      fromEvent<TouchEvent>(document, 'touchcancel').subscribe(this._handleDragStop.bind(this)),
    );
    this._dragSubscriptions.push(
      fromEvent<MouseEvent>(document, 'mouseup').subscribe(this._handleDragStop.bind(this)),
    );
    this._dragSubscriptions.push(
      fromEvent<TouchEvent>(document, 'touchend').subscribe(this._handleDragStop.bind(this)),
    );
  }
  ...
  private _getPositionFromEvent(event: MouseEvent | TouchEvent): number {
    const cursor: any = event.type.includes('touch')
      ? (event as TouchEvent).touches.item(0)
      : event;
