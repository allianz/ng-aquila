...
describe('ComparisonTableFlexRow', () => {
  ...
  describe('responsive', () => {
    ...
    it('should have set placeholder cells correctly + roles (desktop)', () => {
      ...
      const headerPlaceholder = flexRowElements[0].queryAll(
        By.css('.nx-comparison-table__placeholder-cell'),
      );
      expect(headerPlaceholder).toHaveSize(1);

      const contentPlaceholder = flexRowElements[1].queryAll(
        By.css('.nx-comparison-table__placeholder-cell'),
      );
      expect(contentPlaceholder).toHaveSize(0);

      const footerPlaceholder = flexRowElements[2].queryAll(
        By.css('.nx-comparison-table__placeholder-cell'),
      );
