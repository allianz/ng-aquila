...
describe('NxComparisonTableComponent', () => {
  ...
  describe('basic', () => {
    ...
    it('should display the correct number of rows and toggle sections', fakeAsync(() => {
      ...
      const toggleSectionElements = fixture.debugElement.queryAll(
        By.css('.nx-comparison-table__toggle-section'),
      );
      expect(toggleSectionElements).toHaveSize(1);

      const rowsInToggleSections = toggleSectionElements[0].queryAll(
        By.css('.nx-comparison-table__row'),
      );
  ...
  describe('sticky behaviour', () => {
    it('should not cut a top clipping-path on desktop by default', fakeAsync(() => {
      ...
      expect(
        parseInt(regex.exec(tableBody.styles['clip-path'] as string)![1], 10),
    ...
    it('should update top clipping-path when scrolled (desktop)', fakeAsync(() => {
      ...
      const tableBody = fixture.debugElement.query(By.css('.nx-comparison-table__table-body'));
      const regex = /^inset\((.*)px -12px -1px\)$/;
      expect(parseInt(regex.exec(tableBody.styles['clip-path'] as string)![1], 10)).toBeGreaterThan(
        0,
      );
    ...
    it('should update top clipping-path when scrolled (mobile)', fakeAsync(() => {
      ...
      fixture.detectChanges();
      expect(
        parseInt(regex.exec(descriptionCell.styles['clip-path'] as string)![1], 10),
      ).toBeGreaterThan(0);
      expect(toggleSectionHeaderCell.styles['clip-path']).toMatch(/^inset\((0|0px)\)$/);
      ...
      expect(
        parseInt(regex.exec(toggleSectionHeaderCell.styles['clip-path'] as string)![1], 10),
      ).toBeGreaterThan(0);
      flush();
    }));
  ...
  describe('a11y', () => {
    it('should have set the roles correctly (desktop / tablet)', () => {
      ...
      const toggleSectionHeader = toggleSection.query(
        By.css('.nx-comparison-table__toggle-section-header'),
      );
      expect(toggleSectionHeader.attributes.role).toBe('row');

      const toggleSectionBody = toggleSection.query(
        By.css('.nx-comparison-table__toggle-section-body'),
      );
  ...
  describe('hidden column', () => {
    ...
    it('should hide headline cells with popular columns', () => {
      ...
      const querySelector = By.css(
        'nx-comparison-table > div > nx-comparison-table-flex-row .nx-comparison-table__placeholder-cell',
      );
