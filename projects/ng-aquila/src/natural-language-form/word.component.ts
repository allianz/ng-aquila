...
export class NxWordComponent implements AfterContentInit, OnDestroy, OnInit {
  ...
  describedBy = computed<string[]>(() => {
    ...
    const useNaturalLanguageFormDescribedBy =
      this.describedByInput() === undefined && this._nlfErrorWrapper._errorId() !== undefined;
    const nlfError = useNaturalLanguageFormDescribedBy
      ? this._nlfErrorWrapper._errorId()
      : undefined;
  ...
  setupErrorPopover() {
    ...
    (this._overlayState.positionStrategy as FlexibleConnectedPositionStrategy).positionChanges
