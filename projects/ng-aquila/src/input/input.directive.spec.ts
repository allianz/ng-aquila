...
describe('NxInputDirective', () => {
  ...
  it(
    'recognizes never empty inputs like date',
    fakeAsync(() => {
      createTestComponent(TypedAndRequiredInput);
  ...
  describe('formControl', () => {
    it(
      'should update when the form field value is patched without emitting',
      fakeAsync(() => {
...
@Component({
  template: `
    <input nxInput [(ngModel)]="currentValue" required>
  `
})
...
@Component({
  template: `
    <input nxInput [required]="required" [disabled]="disabled" [readonly]="readonly">
  `
})
