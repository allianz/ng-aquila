...
export const enum NxOverlayState { OPEN, CLOSED }
...
export class NxOverlayService implements OnDestroy {
   ...
   */
  open<T, D = any, R = any>(componentOrTemplateRef: ComponentType<T> | TemplateRef<T>, origin: FlexibleConnectedPositionStrategyOrigin,
    config?: NxOverlayConfig<D>): NxOverlayRef<T, R> {
    ...
    const overlayRef = this._attachOverlayContent<T, R>(componentOrTemplateRef,
      overlayContainer,
      cdkOverlayRef,
      config,
      origin);
  ...
  **
  ...
  private _getOverlayConfig(overlayConfig: NxOverlayConfig, origin: FlexibleConnectedPositionStrategyOrigin): OverlayConfig {
    const state = new OverlayConfig({
      ...
      scrollStrategy: overlayConfig.scrollStrategy || this._scrollStrategy(),
  ...
  private _attachOverlayContainer(cdkOverlay: OverlayRef, config: NxOverlayConfig): NxOverlayContainerComponent {
    ...
    const containerPortal = new ComponentPortal(NxOverlayContainerComponent,
  ...
  private _attachOverlayContent<T, R>(
    ...
    origin: FlexibleConnectedPositionStrategyOrigin): NxOverlayRef<T, R> {

    // Create a reference to the overlay we're creating in order to give the user a handle
    // to modify and close it.
    const overlayRef =
      new NxOverlayRef<T, R>(cdkOverlayRef, overlayContainer, origin, this._router, config.id);

    if (componentOrTemplateRef instanceof TemplateRef) {
      overlayContainer.attachTemplatePortal(
        new TemplatePortal<T>(componentOrTemplateRef, null!,
          { $implicit: overlayRef } as any));
    } else {
      const injector = this._createInjector<T>(config, overlayRef, overlayContainer);
      const contentRef = overlayContainer.attachComponentPortal<T>(
  ...
  private _createInjector<T>(
    config: NxOverlayConfig,
    overlayRef: NxOverlayRef<T>,
    overlayContainer: NxOverlayContainerComponent): Injector {
...
function _applyConfigDefaults(
  config?: NxOverlayConfig, defaultOptions?: NxOverlayConfig): NxOverlayConfig {
