...
import {
  addStylesFromDimensions,
  isEmptyArray,
  processSplit,
  validateClassInElement,
} from './utils';
...
export class NxColComponent implements OnInit {
  ...
  set itemSelf(value: ColSelfAlignment | string) {
    ...
    this._alignSelfClasses = value
      ? addStylesFromDimensions(value, MAPPING_ALIGN_SELF)
      : '';
  }
  ...
  checkNotAllowedValues(values: string[], excludes: string[]): boolean {
    ...
    if (found) {
      this.generateError(
        `Exception: NxColDirective. Incorrect parameter. ${found} is not allowed here`
      );
    }
    return !found;
  }
  ...
  validateInput(value: string[]) {
    value.forEach((element) => {
      if (!this.isValueBetween(MIN, MAX, element)) {
        this.generateError(
          `Exception: NxColDirective. Incorrect parameters, values must be between ${MIN} and ${MAX}`
        );
      }
    });
  }
