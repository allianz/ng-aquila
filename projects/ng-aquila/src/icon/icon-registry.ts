...
export const NX_ICON_INITIALIZER = new InjectionToken<Array<(registry: NxIconRegistry) => void>>(
  'NX_ICON_INITIALIZER',
);
...
export class NxIconRegistry implements OnDestroy {
  ...
  addSvgIconLiteral(iconName: string, literal: SafeHtml) {
    this._icons.set(
      iconName,
      new NxSvgIconLiteral(literal, this._httpClient, this._sanitizer, this._document),
    );
  }
  ...
  addSvgIcon(iconName: string, safeUrl: SafeResourceUrl) {
    ...
    this._icons.set(
      iconName,
      new NxSvgIconFromUrl(safeUrl, this._httpClient, this._sanitizer, this._document),
    );
  }
