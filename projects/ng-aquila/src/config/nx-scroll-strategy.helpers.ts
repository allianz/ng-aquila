...
import {
  ...
  NxScrollStrategyOptions,
} from './nx-scroll-strategy.models';

/** Return an appropriate scroll strategy factory from the `Overlay` service. */
export function getScrollStrategyFactory(
  preset: NxScrollStrategy,
  overlay: Overlay,
  options?: NxScrollStrategyOptions,
): () => ScrollStrategy {
  switch (preset) {
    case 'reposition':
      return () =>
...
export function getScrollStrategyDefaultProviders(
  config: NxScrollStrategyDefaultConfig,
): FactoryProvider[] {
  return [
    {
      provide: NX_AUTOCOMPLETE_SCROLL_STRATEGY,
      useFactory: getFactoryFnByDefaultConfig(config, 'autocomplete'),
      deps: [Overlay],
    },
    {
      provide: NX_CONTEXT_MENU_SCROLL_STRATEGY,
      useFactory: getFactoryFnByDefaultConfig(config, 'contextMenu'),
      deps: [Overlay],
    },
    {
      provide: NX_DATEPICKER_SCROLL_STRATEGY,
      useFactory: getFactoryFnByDefaultConfig(config, 'datepicker'),
      deps: [Overlay],
    },
    {
      provide: NX_DROPDOWN_SCROLL_STRATEGY,
      useFactory: getFactoryFnByDefaultConfig(config, 'dropdown'),
      deps: [Overlay],
    },
    {
      provide: NX_NOTIFICATION_PANEL_SCROLL_STRATEGY,
      useFactory: getFactoryFnByDefaultConfig(config, 'notificationPanel'),
      deps: [Overlay],
    },
...
function getFactoryFnByDefaultConfig(
  ...
  return (overlay: Overlay) =>
    getScrollStrategyFactory(
      componentConfig.scrollStrategy,
      overlay,
      componentConfig.scrollStrategyOptions,
    );
}

function getExtraProvidersByDefaultConfig(
  config: NxScrollStrategyDefaultConfig,
): FactoryProvider[] {
  return Object.entries(config.extraComponents ?? {}).map(
    ([componentName, { scrollStrategy, scrollStrategyOptions }]) => ({
      provide: getInjectionToken(componentName),
      useFactory: (overlay: Overlay) =>
        getScrollStrategyFactory(scrollStrategy, overlay, scrollStrategyOptions),
      deps: [Overlay],
    }),
  );
}
...
export function getScrollStrategyFactoryProviders(
  config: NxScrollStrategyFactoryConfig,
): FactoryProvider[] {
  return [
    {
      provide: NX_AUTOCOMPLETE_SCROLL_STRATEGY,
      useFactory: getFactoryFnByFactoryConfig(config, 'autocomplete'),
      deps: [Overlay],
    },
    {
      provide: NX_CONTEXT_MENU_SCROLL_STRATEGY,
      useFactory: getFactoryFnByFactoryConfig(config, 'contextMenu'),
      deps: [Overlay],
    },
    {
      provide: NX_DATEPICKER_SCROLL_STRATEGY,
      useFactory: getFactoryFnByFactoryConfig(config, 'datepicker'),
      deps: [Overlay],
    },
    {
      provide: NX_DROPDOWN_SCROLL_STRATEGY,
      useFactory: getFactoryFnByFactoryConfig(config, 'dropdown'),
      deps: [Overlay],
    },
    {
      provide: NX_NOTIFICATION_PANEL_SCROLL_STRATEGY,
      useFactory: getFactoryFnByFactoryConfig(config, 'notificationPanel'),
      deps: [Overlay],
    },
...
function getExtraProvidersByFactoryConfig(
  config: NxScrollStrategyFactoryConfig,
): FactoryProvider[] {
  return Object.entries(config.extraComponents ?? {}).map(
    ([componentName, { scrollStrategyFactory }]) => ({
      provide: getInjectionToken(componentName),
