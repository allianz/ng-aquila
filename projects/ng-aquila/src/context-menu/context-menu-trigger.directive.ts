...
export const NX_CONTEXT_MENU_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'nx-context-menu-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
 ...
 */
export function NX_CONTEXT_MENU_SCROLL_STRATEGY_PROVIDER_FACTORY(
  overlay: Overlay,
): () => ScrollStrategy {
  return () => overlay.scrollStrategies.reposition();
}
...
export class NxContextMenuTriggerDirective implements AfterContentInit, OnDestroy {
  ...
  @Input() set scrollStrategy(value: NxContextMenuScrollStrategy | null | undefined) {
    if (this.#scrollStrategy !== value) {
      ...
      this._scrollStrategyFactory = value
        ? this.getScrollStrtegyFactory(value)
  ...
  constructor(
    ...
    @Optional()
    @Self()
    private readonly _contextMenuItemInstance: NxContextMenuItemComponent | null,
  ...
  openContextMenu(origin?: FocusOrigin, position?: Point): void {
    ...
    if (this._triggerButton) {
      ...
      this.contextMenu.closed
        .pipe(take(1))
        .subscribe(() => this._triggerButton?.setTriggerInactive());
    }
  ...
  private _contextMenuClosingActions() {
    ...
    const parentClose = this._parentMenu
      ? (this._parentMenu.closed as Observable<void | 'click' | 'keydown' | 'tab'>)
      : observableOf();
