...
export const NX_MESSAGE_TOAST_DEFAULT_CONFIG = new InjectionToken<NxMessageToastConfig>(
  'NX_MESSAGE_TOAST_DEFAULT_CONFIG',
);

/** Injection token that can be used to access the data that was passed in to a message toast. */
export const NX_MESSAGE_TOAST_COMPONENT_DATA = new InjectionToken<any>(
  'NxMessageToastComponentData',
);
...
export class NxMessageToastService implements OnDestroy {
  ...
  openFromTemplate(template: TemplateRef<any>, config?: NxMessageToastConfig): NxMessageToastRef {
    ...
    const portal = new TemplatePortal(template, null!, {
      $implicit: currentConfig.data,
      toastRef,
    } as any);
