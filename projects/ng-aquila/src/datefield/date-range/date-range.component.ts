    ...
    NxDateRangeConnector
{
  ...
  private readonly _startDateDirective =
    viewChild.required<NxDatefieldDirective<D>>('startDateFieldDirective');
  ...
  private readonly _endDateDirective =
    viewChild.required<NxDatefieldDirective<D>>('endDateFieldDirective');
   ...
   */
  readonly datepicker: InputSignal<NxDatepickerComponent<D> | undefined> = input<
    NxDatepickerComponent<D> | undefined
  >();
  ...
  constructor(
    ...
    this._dynamicStartDateWidthHook = afterEveryRender(() => {
      ...
      const newWidth =
        metrics.width + parseInt(styles.paddingRight, 10) + parseInt(styles.paddingLeft, 10) + 1;
  ...
  ngDoCheck(): void {
    if (
      ...
      (!this.datepicker()?.opened || this.ngControl.valid || !this.datepicker())
    ) {
