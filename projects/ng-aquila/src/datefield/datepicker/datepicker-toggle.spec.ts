...
describe('NxDatepickerToggleComponent', () => {
  ...
  it('should open datepicker on Enter keydown and focus on active date', fakeAsync(() => {
    ...
    const activeDateElement = document.body.querySelector(
      '.nx-calendar-body-active',
    ) as HTMLElement;
...
describe('NxDatepickerToggleComponent using injection token', () => {
  ...
  it('changes the tabindex on injection token change', inject(
    [DATEPICKER_DEFAULT_OPTIONS],
    (defaultOptions: DatepickerDefaultOptions) => {
      createTestComponent(BasicToggleDateComponent);
