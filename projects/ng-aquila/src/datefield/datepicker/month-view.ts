...
export class NxMonthViewComponent<D> implements AfterContentInit {
  ...
  @Input() set selected(value: D | DateRange<D> | null) {
    ...
    this.selectedDateInMonth.set(
      this._getDateInCurrentMonth((value as DateRange<D>)?.start || (this._selected as D)),
    );
    if (this.isRange()) {
      this.selectedEndDate.set(
        this._getDateInCurrentMonth((value as DateRange<D>)?.end || (this._selected as D)),
      );
  ...
  handleHoverChange(date: number, monthsToAdd = 0) {
    const selectedYear = this._dateAdapter.getYear(
      this._dateAdapter.addCalendarMonths(this.activeDate, monthsToAdd),
    );
    const selectedMonth = this._dateAdapter.getMonth(
      this._dateAdapter.addCalendarMonths(this.activeDate, monthsToAdd),
    );
  ...
  _init() {
    const initDateStart: D | null =
      this.selected && this.isRange()
    ...
    if (this.isRange() && this.selected) {
      this.selectedEndDate.set(
        this._getDateInCurrentMonth((this.selected as DateRange<D>)?.end || (this.selected as D)),
      );
  ...
  private _createWeekCells() {
    ...
    for (let i = 0, cell = this._firstWeekOffset; i < daysInMonth; i++, cell++) {
      ...
      this._weeks[this._weeks.length - 1].push(
        new NxCalendarCell(i + 1, dateNames[i], ariaLabel, enabled),
      );
    }
  }
  ...
  private getRangeInMonth(value: DateRange<D>): number[] {
    ...
    const isCompleteRange =
      value.start &&
      value.end &&
      this._dateAdapter.isValid(value.start!) &&
      this._dateAdapter.isValid(value.end!);
    if (!isCompleteRange) {
      return [];
    }
    ...
    const currentMonthIsLaterThenStart =
      this._dateAdapter.compareDate(this.activeDate, value.start!) > 0;
    const currentMonthIsEarlierThenEnd =
      this._dateAdapter.compareDate(this.activeDate, value.end!) < 0;
  ...
  private _getValidDateOrNull(obj: any): D | DateRange<D> | null {
    if (this.isRange() && !this._dateAdapter.isDateInstance(obj)) {
      ...
      return {
        start:
          this._dateAdapter.isDateInstance(dateRangeObject?.start) &&
          this._dateAdapter.isValid(dateRangeObject!.start!)
            ? obj.start
            : null,
        end:
          this._dateAdapter.isDateInstance(dateRangeObject?.end) &&
          this._dateAdapter.isValid(dateRangeObject!.end!)
            ? obj.end
            : null,
      };
    }
