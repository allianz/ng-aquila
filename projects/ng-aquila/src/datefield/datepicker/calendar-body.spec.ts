...
describe('NxCalendarBodyComponent', () => {
  ...
  describe('standard calendar body', () => {
    ...
    it('cell should be selected on click', () => {
      const todayElement =
          calendarBodyNativeElement.querySelector('.nx-calendar-body-today') as HTMLElement;
      todayElement.click();
      fixture.detectChanges();

      expect(todayElement.classList)
          .toContain('nx-calendar-body-selected', 'today should be selected');
    });
  ...
  describe('calendar body with previous and following items', () => {
    ...
    it('should have the correct number of adjacent items', () => {
      const followingItemsCount = testComponent.previousItems.length
...
@Component({
  template: `<table nx-calendar-body
                    [label]="label"
                    [rows]="rows"
