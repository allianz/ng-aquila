...
describe('NxFormfieldHarness', () => {
  ...
  it('should get prefix', async () => {
    const { loader } = createTestComponent(
      `<nx-formfield><span nxFormfieldPrefix>The Prefix</span><input nxInput></nx-formfield>`,
    );
    const formfield = await loader.getHarness(NxFormfieldHarness);
  ...
  it('should get suffix', async () => {
    const { loader } = createTestComponent(
      `<nx-formfield><input nxInput><span nxFormfieldSuffix>The Suffix</span></nx-formfield>`,
    );
    const formfield = await loader.getHarness(NxFormfieldHarness);
  ...
  it('should get appendix', async () => {
    const { loader } = createTestComponent(
      `<nx-formfield><input nxInput><span nxFormfieldAppendix>The Appendix</span></nx-formfield>`,
    );
    const formfield = await loader.getHarness(NxFormfieldHarness);
  ...
  it('should get hint', async () => {
    const { loader } = createTestComponent(
      `<nx-formfield><input nxInput><span nxFormfieldHint>The Hint</span></nx-formfield>`,
    );
    const formfield = await loader.getHarness(NxFormfieldHarness);
  ...
  describe('filters', () => {
    ...
    it('should find by hasErrors', async () => {
      ...
      control.markAllAsTouched();
      expect(await loader.getAllHarnesses(NxFormfieldHarness.with({ hasErrors: true }))).toHaveSize(
        1,
      );
      expect(
        await loader.getAllHarnesses(NxFormfieldHarness.with({ hasErrors: false })),
      ).toHaveSize(1);
    });
