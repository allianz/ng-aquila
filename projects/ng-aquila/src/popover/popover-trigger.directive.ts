...
export const NX_POPOVER_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'nx-popover-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
 ...
 */
export function NX_POPOVER_SCROLL_STRATEGY_PROVIDER_FACTORY(
  overlay: Overlay,
): () => ScrollStrategy {
  return () => overlay.scrollStrategies.close();
}
...
export const POPOVER_DEFAULT_OPTIONS = new InjectionToken<PopoverDefaultOptions>(
...
export class NxPopoverTriggerDirective implements AfterViewInit, OnDestroy {
  ...
  private readonly _possiblePopoverDirections: PopoverDirection[] = [
    'bottom',
    'top',
    'left',
    'right',
  ...
  @Input('nxPopoverScrollStrategy') set scrollStrategy(
    value: PopoverTriggerScrollStrategy | null | undefined,
  ) {
    if (this.#scrollStrategy !== value) {
      this.#scrollStrategy = value;
      this._scrollStrategyFactory = value
        ? this.getScrollStrategyFactory(value)
        : this._defaultScrollStrategyFactory;
  ...
  constructor(
    ...
    @Optional()
    @Inject(POPOVER_DEFAULT_OPTIONS)
    private readonly _defaultOptions: PopoverDefaultOptions | null,
  ...
  private _getFallbackPositions(
    ...
    const remainigDirections = possibleDirections.filter(
      (possibleDirection) => possibleDirection !== direction,
    );
    ...
    const nextFallbackPosition = remainigDirections.includes(inverseDirection)
      ? inverseDirection
      : possibleDirections[0];
    return [
      ...fallbackPositions,
      ...this._getFallbackPositions(nextFallbackPosition, remainigDirections),
    ];
  }
  ...
  private _getHorizontalFallbackPositionPairs(
    direction: PopoverHorizontalDirection,
  ): ConnectionPositionPair[] {
