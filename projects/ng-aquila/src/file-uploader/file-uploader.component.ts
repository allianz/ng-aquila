...
{
  /** @docs-private */
  @ContentChild(NxFileUploaderButtonDirective, { static: false })
  button?: NxFileUploaderButtonDirective;

  /** @docs-private */
  @ContentChild(NxFileUploaderDropZoneComponent, { static: true })
  ...
  /** @docs-private */
  @ContentChildren(NxFileUploaderHintDirective)
    ...
    if (this._value !== value) {
      if (value) {
        if (this._filesSubscriptions) {
          this._filesSubscriptions.forEach((fileSubscription: Subscription) =>
  ...
  ngAfterContentInit(): void {
    ...
    this.button._clicked.pipe(takeUntil(this._destroyed)).subscribe(() => {
      ...
      const reachMaxFileNumber =
        this.maxFileNumber && (this.value?.length || 0) === this.maxFileNumber;
  ...
  uploadFiles() {
    if (!this.uploader) {
      console.warn(
        'No uploader found. Please define a NxFileUpload to use the uploading mechanism.',
      );
      return;
    }
