...
export class SchematicTestSetup {
  ...
  init(): void {
    beforeEach(async () => {
      ...
      this.appTree = await createApp(this.runner, workspaceTree, {
        name: this.appTreeName,
        standalone: false,
      });
      this.createFileDir(this.appTree, this.appTreeName);

      // create standalone app
      this.appTreeStandalone = await createApp(this.runner, workspaceTree, {
        name: this.appTreeNameStandalone,
        standalone: true,
      });
  ...
  createFileDir(testApp: UnitTestTree, appName: string): void {
    ...
    const testAppTsconfig = JSON.parse(
      testApp.readContent(testAppTsconfigPath).replace(/\/\*.*\*\//g, ''),
    );
