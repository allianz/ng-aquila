...
export const getSortedCountryCodes = (countryNames: LocalizedCountryNames<any>) =>
  Object.keys(countryNames)
    ...
    .sort((a, b) => {
      ...
      const localA: string = (
        Array.isArray(countryNames[a]) ? countryNames[a][0] : countryNames[a]
      ) as string;
      const localB: string = (
        Array.isArray(countryNames[b]) ? countryNames[b][0] : countryNames[b]
      ) as string;
