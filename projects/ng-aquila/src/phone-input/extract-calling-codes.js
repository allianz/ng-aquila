...
if (!meta || !meta.country_calling_codes || meta.country_calling_codes.length === 0) {
  throw new Error(
    'Error extracting calling codes from libphonenumber-js. Could not read metadata or it was empty.',
  );
}
...
if (!metadata.callingCodes['49'] || !metadata.callingCodes['49'].includes('DE')) {
  throw new Error(
    `Couldn't find entry for german calling code. There might be something wrong with the extraction from libphonenumber-js`,
  );
}
