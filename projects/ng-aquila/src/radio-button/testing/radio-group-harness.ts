...
export class NxRadioGroupHarness extends ComponentHarness {
    ...
    static with<T extends NxRadioGroupHarness>(this: ComponentHarnessConstructor<T>, options: NxRadioGroupHarnessFilters = {}) {
        return new HarnessPredicate<NxRadioGroupHarness>(this, options)
            .addOption('text', options.label, (harness, label) => HarnessPredicate.stringMatches(harness.getLabel(), label))
    ...
    async clickOption(filter?: NxRadioHarnessFilters): Promise<void> {
        ...
        if (items.length === 0) {
            ...
            const unfilteredItemTexts = (await parallel(() => unfilteredItems.map(o => o.getLabel()))).join(', ');
            throw Error(`RadioGroup does not have options matching the specified filter, available items: ${unfilteredItemTexts}.`);
        }
