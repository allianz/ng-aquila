...
export class NxPaginationUtils {
  ...
  getSlides(totalSlides: number): any[] {
    return Array.from(Array(totalSlides).keys()).map((item) =>
      this.createPaginationItem(item + 1, item + 1),
    );
  }
  ...
  getMobilePages(currentPage: number, totalPages: number): any[] {
    ...
    if (totalPages < this._pagesMobile) {
      return Array.from(Array(totalPages).keys()).map((item) =>
        this.createPaginationItem(item + 1, item + 1),
      );
    }
    ...
    switch (currentPage) {
      ...
      // last page active
      case totalPages:
        return [totalPages - 2, totalPages - 1, totalPages].map((item) =>
          this.createPaginationItem(item, item),
        );
      // any other page active
      default:
        return [currentPage - 1, currentPage, currentPage + 1].map((item) =>
          this.createPaginationItem(item, item),
        );
    }
  }
