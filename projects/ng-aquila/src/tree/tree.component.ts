...
export class NxTreeComponent<T> extends CdkTree<T> implements OnDestroy, OnInit {
  ...
  constructor(private _wrapperDiffers: IterableDiffers,
  ...
  _getNextVisibleNode(): T | undefined {
    ...
    if (this.childrenMap.has(this._focusedData) &&
        this.treeControl && this.treeControl.isExpanded(this._focusedData)) {
  ...
  _getLastChild(targetNode: T) {
    ...
    while (currentData && this.childrenMap.has(currentData) &&
