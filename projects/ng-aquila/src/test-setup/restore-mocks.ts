 ...
 * across specs — which is what made `grid/col.component.spec.ts` intermittently
 * see the `nxRow` warning emitted by the preceding spec.
 *
 ...
 * collected at that moment. Measured on a 30-file run: the module was evaluated once and a
 * module-scope hook fired for 18 of 647 tests, all in a single file. The config flag has no
 * such limitation: the runner applies it itself in `onBeforeTryTask`, ahead of every test.
 * See `viewport-shim.ts`, which had the same defect and needed a different fix.
 *
