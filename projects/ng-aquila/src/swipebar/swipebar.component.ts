...
export class NxSwipebarComponent implements AfterViewInit {
  ...
  private _onScroll($event: Event) {
    ...
    this._position = Math.floor(
      (target.scrollLeft / (scrollWidth - clientWidth)) * (target.offsetWidth - this._barLength),
    );
    this._updateIndicator();
  }
  ...
  private _onMousemove($event: MouseEvent) {
    ...
    this._position = Math.max(
      0,
      Math.min(offsetWidth - this._barLength, this._position + $event.clientX - this._startX),
    );
    this._startX = $event.clientX;
  ...
  private _updateScrollablePosition() {
    requestAnimationFrame(() => {
      ...
      this._scrollable.nativeElement.scrollLeft =
        (this._position / (offsetWidth - this._barLength)) * (scrollWidth - clientWidth);
    });
  }
