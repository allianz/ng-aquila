...
export class NxTabComponent implements OnChanges, OnDestroy, AfterContentInit {
   ...
   * @docs-private
   */
  @ContentChild(NxTabLabelDirective, { read: TemplateRef, static: true }) set templateLabel(
  ...
  constructor(
    @Inject(NxTabGroupBase) @Optional() @Host() private readonly _tabGroup: NxTabGroupBase | null,
  ) {
    if (!this._tabGroup) {
