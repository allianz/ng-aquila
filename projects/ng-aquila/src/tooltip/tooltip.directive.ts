...
export const NX_TOOLTIP_SCROLL_STRATEGY = new InjectionToken<() => ScrollStrategy>(
  'nx-tooltip-scroll-strategy',
  {
    providedIn: 'root',
    factory: () => {
 ...
 */
export function NX_TOOLTIP_SCROLL_STRATEGY_PROVIDER_FACTORY(
  overlay: Overlay,
): () => ScrollStrategy {
  return () => overlay.scrollStrategies.reposition();
}
...
export const NX_TOOLTIP_DEFAULT_OPTIONS = new InjectionToken<NxTooltipDefaultOptions>(
  'nx-tooltip-default-options',
  {
    providedIn: 'root',
...
export class NxTooltipDirective implements OnDestroy, OnInit, AfterViewInit {
  ...
  private readonly _possibleTooltipPositions: TooltipPosition[] = [
    'bottom',
    'top',
    'left',
    'right',
  ...
  constructor(
    ...
    private readonly _defaultOptions: NxTooltipDefaultOptions | null,
  ...
  _updateSelectabilityStyles(): void {
    ...
    const isSelectable =
      element.nodeName === 'INPUT' || element.nodeName === 'TEXTAREA' || this._selectable;
    const userSelect = isSelectable ? 'auto' : 'none';
    elementStyle.webkitUserSelect =
      elementStyle.userSelect =
      (elementStyle as any).msUserSelect =
        userSelect;
  }
  ...
  private _getFallbackPositions(
    ...
    const remainigPositions = possiblePositions.filter(
      (possiblePosition) => possiblePosition !== position,
    );
    ...
    const nextFallbackPosition = remainigPositions.includes(inversePosition)
      ? inversePosition
      : possiblePositions[0];
    return [
      ...fallbackPositions,
      ...this._getFallbackPositions(nextFallbackPosition, remainigPositions),
    ];
  }
  ...
  private _getHorizontalFallbackPositionPairs(
    position: TooltipHorizontalPosition,
  ): ConnectionPositionPair[] {
