...
export class NxExpandableTableDirective implements OnDestroy, AfterViewInit, NxExpandable {
  @ContentChildren(NxExpandableTableRowComponent, { descendants: true })
  rows!: QueryList<NxExpandableTableRowComponent>;
  ...
  ngAfterViewInit(): void {
    this.rows.changes
      .pipe(
        startWith(this.rows),
        mergeMap((rows: NxExpandableTableRowComponent[]) =>
          combineLatest(rows.map((row) => row.expanded)),
        ),
