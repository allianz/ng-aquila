...
export class NxSwipebarComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      this._indicator.nativeElement.addEventListener(
        'mousedown',
        this._onMousedown
      );
    });
  ...
  private _onScroll($event: Event) {
    ...
    this._position = Math.floor(
      (target.scrollLeft / (scrollWidth - clientWidth)) *
        (target.offsetWidth - this._barLength)
    );
    this._updateIndicator();
  }
  ...
  private _onMousemove($event: MouseEvent) {
    ...
    this._position = Math.max(
      0,
      Math.min(
        offsetWidth - this._barLength,
        this._position + $event.clientX - this._startX
      )
    );
    this._startX = $event.clientX;
  ...
  private _updateScrollablePosition() {
    requestAnimationFrame(() => {
      ...
      this._scrollable.nativeElement.scrollLeft =
        (this._position / (offsetWidth - this._barLength)) *
  ...
  private _updateIndicator() {
    requestAnimationFrame(() => {
      ...
      this._indicator.nativeElement.style.transform = `translateX(${
        this._position
      }px)`;
