...
export class NxMask {
  ...
  updateConfig(maskConfig: NxMaskConfig, { callOnChange = true, updateValue = true } = {}) {
    ...
    if (updateValue) {
      const newValue = this.maskConfig.deactivateMask
        ? this.element.value
        : this.getMaskedString(this.element.value);
      this.updateValue(newValue, callOnChange);
    }
  }

  private addListener<E extends keyof HTMLElementEventMap>(
    eventType: E,
  ...
  getUnmaskedValue(value: string): string {
    const unmaskedValue = this.maskConfig?.separators?.reduce(
      (unmasked, separator) => unmasked.split(separator).join(''),
      value,
    );
  ...
  private onInput(event: Event) {
    ...
    if (this._cursor?.position !== undefined) {
      ...
      if (oldVal === input.value) {
        ...
        const newPosition =
          this._cursor.selectionStart + this._calculateCursorShift(this._cursor.selectionStart);
  ...
  private onPaste(event: ClipboardEvent) {
    ...
    if (
      input.value.length === this.maskConfig.mask.length &&
    ...
    const pastedUnmaskedValue = this.maskConfig.separators!.reduce(
      (unmasked, separator) => unmasked.split(separator).join(''),
      maskedString,
    );
  ...
  private handleDelete(event: KeyboardEvent) {
    ...
    const lastCharacter = currentValue.substring(
      ...
      input.selectionEnd! - backspaceShift + 1,
    );
    const selectionAtLastCharacter =
      input.selectionStart === currentValue.length - 1 + backspaceShift;
    if (input.selectionStart !== input.selectionEnd) {
      ...
      input.setSelectionRange(
        input.selectionStart! - backspaceShift,
        input.selectionEnd! - backspaceShift,
      );
      event.preventDefault();
    } else {
  ...
  getMaskedString(inputValue: string, maskStartIndex = 0): string {
    ...
    while (inputIndex < transformedValue.length) {
      ...
      if (
        this._isStringAllowed(
          transformedValue[inputIndex],
          this.maskConfig.mask[maskIndex] as MASK_TYPE,
        )
      ) {
