...
export class ManifestService {
  ...
  constructor(
    @Optional() @Inject(NXV_MANIFEST_TOKEN) private readonly initialManifest: Manifest | null,
  ) {
    this.init();
  }
  ...
  init() {
    this.update({
      ...(this.initialManifest ?? { api: [], components: [], examples: [], guides: [] }),
    });
  }
  ...
  isEmpty() {
    const available =
      this._current !== null &&
      Object.keys(this._current).length === 0 &&
      this._current.constructor === Object;
    return available;
  }
  ...
  private getGroupedComponents(): Category[] {
    ...
    // form datastructure
    const groupedCategories = Object.entries(componentsDict).reduce<Category[]>(
    ...
    groupedCategories.map((components: { children: any[] }) =>
      components.children.sort(sortByLabel),
    );
